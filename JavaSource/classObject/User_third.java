...
public class User_third implements Runnable{
	...
	int user_type=3;//用户类型
	
	double percent=0.8;		//证券价值占有所有资金的比例  

	public double Sensity_Politic=0.7;//这个怎么用还不清楚  政策敏感度  用于买的计算
	
	double ativefund=500000;//帐户的可用资金
    
	double AccountValue=0;//帐户的股票价值
    
	double  Total=500000;//用户的总资产
    
	double  percent_earn=0;//用户的收益率
	public int idsty=3;
	public static long hold_time=100;//原来为45*24*60*60*1000
	public void run() 
	...
	public void init()
			...
			int t;
						...
						if(t==0)
						{ 
							System.out.println("类型'理智'的用户："+account_id+"注册"); 
